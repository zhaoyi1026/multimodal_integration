...
set.seed(100)
# generate X from a Bernoulli distribution
X<-rbinom(n,1,prob=0.5)

# M1
eps.M1<-rmvnorm(n,mean=rep(0,p1),sigma=Sigma1)
M1<-X%*%beta+eps.M1

# M2
eps.M2<-rmvnorm(n,mean=rep(0,p2),sigma=Sigma2)
M2<-X%*%zeta+M1%*%Lambda+eps.M2

# Y
eps.Y<-rnorm(n,mean=0,sd=sigma.Y)
Y<-X*delta+M1%*%theta+M2%*%pi+eps.Y
##################################
